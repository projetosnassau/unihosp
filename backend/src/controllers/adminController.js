...
      return res.status(404).json({ error: "Admin não encontrado com este email." });
...
    res.json({ token, admin: {id: admin.id, name: admin.name, email: admin.email, tipo: 'admin'} });
...
        return res.status(404).json({ error: "Admin não encontrado para atualização." });
...
      return res.status(403).json({ error: "Um administrador não pode se auto-deletar." });
...
        return res.status(404).json({ error: "Admin não encontrado para deleção." });
